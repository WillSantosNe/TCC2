...
                });
                ...
                // --- Lógica para mover o dropdown ao abrir/fechar (COPIAR AQUI) ---
                // Remove listeners anteriores para evitar duplicação (se já existirem)
                $('#tabelaProvas tbody').off('show.bs.dropdown move.dropdown'); // Use '#tabelaProvas'
                ...
                $('body').on('hide.bs.dropdown move.dropdown', '.dropdown-menu', function (e) {
                    ...
                    if ($originalParent && !$dropdownMenu.parent().is($originalParent)) {
                        ...
                        $dropdownMenu.removeData('bs-dropdown-original-parent'); // Limpe a referência após mover
                    }
                });
                // --- FIM Lógica para mover o dropdown (COPIADA AQUI) ---
...
            $(trElement).data('status', 'Concluída');
        }
...
            const dropdownHtml = `
                <div class="dropdown">
                    <button class="btn btn-sm btn-icon" type="button" data-bs-toggle="dropdown" aria-expanded="false" aria-label="Ações da prova" data-bs-popper-boundary="window"> {/* <-- ADICIONADO AQUI */}
